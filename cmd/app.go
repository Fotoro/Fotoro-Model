...
func RunApp(dbPath, model string) {
	...
	if resp, err := http.Get(healthURL); err == nil {
		resp.Body.Close()
		backendRunning = true
		fmt.Println("[INIT] Backend already running at", addr)
	}
	...
	if !backendRunning {
		...
		fmt.Println("[INIT] Backend started on", addr)
	...
	if guiPath == "" {
		...
		if err := buildCmd.Run(); err != nil {
			fmt.Printf("[WARN] Could not build GUI. Server is running at %s\n", baseURL)
			fmt.Println("[INFO] Use the API directly or open the website dashboard")
			select {}
		}
		guiPath = "./fotoro-gui"
	}
	...
	fmt.Printf("[INIT] GUI closed. Backend still running on %s\n", addr)
	fmt.Printf("[INFO] Stop with: kill $(lsof -t -i:%s)\n", strings.TrimPrefix(addr, "127.0.0.1:"))
}
